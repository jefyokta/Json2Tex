...
class LatexConverter implements InterfaceConverter
{
    ...
    {
        return "\\par " . Converter::setContent($node->content) . "\n";
    }
    /**
     * 
    ...
    function heading($content)
    {
        ...
        return "\\$synt{" . Converter::setContent($content->content) . "}\n";
    }
    ...
    {
        $items = Converter::setContent($content->content)->getLatex();
    ...
    {
        return "\\item " . Converter::setContent($content->content)->getLatex() . "\n";
    }
    ...
    function table($element): string
    {
        ...
        foreach ($contents as $rowIndex => $row) {
            ...
            foreach ($row->content as $cell) {
                ...
                $text = Converter::setContent($contents);
                $colspan = $cell->attrs->colspan ?? 1;
